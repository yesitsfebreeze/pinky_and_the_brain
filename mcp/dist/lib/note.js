...
exports.parseNotePool = parseNotePool;
exports.serializeNote = serializeNote;
exports.serializeNotePool = serializeNotePool;
exports.today = today;
// ---------------------------------------------------------------------------
// Parse
// ---------------------------------------------------------------------------
 ...
 */
function parseNotePool(content) {
...
function serializeNote(note) {
    ...
    const metaParts = [`rating: ${note.rating}`];
    if (note.created)
        metaParts.push(`created: ${note.created}`);
    if (note.lastUsed)
        metaParts.push(`last_used: ${note.lastUsed}`);
    if (note.concepts.length > 0)
        metaParts.push(`concepts: ${note.concepts.join(', ')}`);
    if (note.relatedNotes.length > 0)
        metaParts.push(`related-notes: ${note.relatedNotes.join(', ')}`);
 ...
 */
function serializeNotePool(notes) {
