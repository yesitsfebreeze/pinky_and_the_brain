...
import { readThoughts } from '../lib/storage.js';
...
function parseConceptsFile(content: string): Map<string, ConceptNode> {
...
function expandConcepts(seeds: string[], graph: Map<string, ConceptNode>, depth: number): Set<string> {
...
function recencyBonus(note: Note): number {
...
function repoMatchBonus(note: Note, sourceRoot: string): number {
  for (const src of note.sources) {
    if (fs.existsSync(path.join(sourceRoot, src))) return 15;
  }
  return 0;
}
...
function textScore(note: Note, query: string): number {
...
export async function query(config: PatbConfig, args: QueryArgs): Promise<QueryResult> {
  ...
  const conceptsPath = path.join(config.brainRoot, 'concepts.md');
  const conceptsContent = fs.existsSync(conceptsPath)
  ...
  const scored = pool
    .map(note => {
      ...
      if (textMatch < TEXT_THRESHOLD) return null;
