...
export interface ForgetArgs {
  query: string;
  confirmed?: boolean;
  /** If provided, only remove notes whose titles appear in this list */
  ids?: string[];
}

export interface ForgetResult {
  matches: Array<{ title: string; rating: number; snippet: string }>;
  removed: number;
  message: string;
}

function scoreMatch(note: Note, query: string): number {
...
export async function forget(config: PatbConfig, args: ForgetArgs): Promise<ForgetResult> {
  ...
  await syncAndPush(config.brainRoot, `pb: forget - ${args.query}`);
